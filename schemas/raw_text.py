...
class DocumentLayoutType(str, Enum):
    FREE_FORM_PAD = "free_form_pad"        # E.g., doctor prescription pad with freehand cursive
    STRUCTURED_FORM = "structured_form"    # E.g., bilingual pre-printed checklist / hospital template
    MIXED = "mixed"
    UNKNOWN = "unknown"
...
class ExtractedRegion(BaseModel):
    ...
    confidence: float = Field(default=1.0, ge=0.0, le=1.0, description="OCR/VLM confidence score (0-1)")
    is_handwritten: bool = Field(default=True, description="True if handwritten, False if printed")
    has_strikethrough: bool = Field(default=False, description="True if text shows visible strike-through/corrections")
...
class RawPrescriptionText(BaseModel):
    ...
    image_uri: Optional[str] = Field(default=None, description="URI or path of the deidentified image")
    raw_text: str = Field(..., description="Full concatenated transcribed text of the prescription")
    ...
    regions: List[ExtractedRegion] = Field(default_factory=list, description="Segmented text blocks/regions")
