...
uploaded_file = st.file_uploader(
    "Upload Prescription Image", type=["jpg", "jpeg", "png", "pdf"]
)
            ...
            with st.spinner("Analyzing with Vision Language Model..."):
                try:
                    ...
                    response = requests.post(
                        "http://localhost:8000/api/v1/extract", files=files
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        if medications:
                            ...
                            for med in medications:
                                with st.container():
                                    st.markdown(
                                        f"**{med.get('medicine_name', 'Unknown')}**"
                                    )
                        ...
                        st.error(
                            f"Error processing prescription. Backend returned status code: {response.status_code}"
                        )
                        st.write(response.text)
                except requests.exceptions.ConnectionError:
                    st.error(
                        "Could not connect to the backend API. Is it running on port 8000?"
                    )
